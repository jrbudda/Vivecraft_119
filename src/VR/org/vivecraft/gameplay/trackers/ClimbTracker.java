...
public class ClimbTracker extends Tracker
{
    ...
    public boolean isActive(LocalPlayer p)
    {
        ...
        else if (!this.mc.vrPlayer.getFreeMove() && !Minecraft.getInstance().vrSettings.simulateFalling)
        {
            return false;
        }
        else if (!this.mc.vrSettings.realisticClimbEnabled)
        {
            return false;
        }
    ...
    public void idleTick(LocalPlayer player)
    {
        ...
        this.mc.vr.getInputAction(this.mc.vr.keyClimbeyGrab).setEnabled(ControllerType.RIGHT, this.isClimbeyClimb() && (this.isGrabbingLadder() || this.inblock[0] || this.forceActivate));
        this.mc.vr.getInputAction(this.mc.vr.keyClimbeyGrab).setEnabled(ControllerType.LEFT, this.isClimbeyClimb() && (this.isGrabbingLadder() || this.inblock[1] || this.forceActivate));
    }
    ...
    public void doProcess(LocalPlayer player)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            avec3[i] = this.mc.vrPlayer.vrdata_world_pre.getController(i).getPosition();
            Vec3 vec3 = this.mc.vrPlayer.vrdata_world_pre.getController(i).getDirection();
            ...
            {
                Vec3 vec31 = this.mc.vrPlayer.vrdata_world_pre.getController(i).getPosition().subtract(vec3.scale(0.2D));
            ...
            if (!this.latched[i] && !flag && aboolean1[i])
            {
                ...
                if (!this.wasinblock[i] && this.inblock[i] && aboolean[i] || !this.wasbutton[i] && aboolean[i] && this.inblock[i])
                {
                    ...
                    if (!flag3)
                    {
                        this.mc.vrPlayer.blockDust(this.latchStart[i].x, this.latchStart[i].y, this.latchStart[i].z, 5, blockpos, blockstate, 0.1F, 0.2F);
                    }
                }
            }
        ...
        if (!this.latched[0] && !this.latched[1])
        {
            for (int k = 0; k < 2; ++k)
            {
                if (this.inblock[k] && aboolean[k] && aboolean1[k])
                {
                    ...
                    this.latchStart_room[k] = this.mc.vrPlayer.vrdata_room_pre.getController(k).getPosition();
                    this.latchStartBody[k] = player.position();
                    this.latchStartController = k;
                    this.latched[k] = true;
                    this.latchbox[k] = this.box[k];
                    this.wantjump = false;
                    this.mc.vr.triggerHapticPulse(k, 2000);
                    BlockPos blockpos4 = new BlockPos(this.latchStart[k]);
                    BlockState blockstate4 = this.mc.level.getBlockState(blockpos4);

                    if (!flag3)
                    {
                        this.mc.vrPlayer.blockDust(this.latchStart[k].x, this.latchStart[k].y, this.latchStart[k].z, 5, blockpos4, blockstate4, 0.1F, 0.2F);
                    }
                }
            }
        }
        ...
        {
            this.wantjump = this.mc.vr.keyClimbeyJump.isDown() && this.mc.jumpTracker.isClimbeyJumpEquipped();
        }
        ...
        else
        {
            if ((this.latched[0] || this.latched[1]) && this.rand.nextInt(20) == 10)
            {
                ...
                if (!flag3)
                {
                    this.mc.vrPlayer.blockDust(this.latchStart[this.latchStartController].x, this.latchStart[this.latchStartController].y, this.latchStart[this.latchStartController].z, 1, blockpos3, blockstate3, 0.1F, 0.2F);
                }
            }

            Vec3 vec34 = this.mc.vrPlayer.vrdata_world_pre.getController(this.latchStartController).getPosition();
            VRPlayer vrplayer = this.mc.vrPlayer;
            Vec3 vec35 = VRPlayer.room_to_world_pos(this.latchStart_room[this.latchStartController], this.mc.vrPlayer.vrdata_world_pre);
            ...
            if (!flag2)
            {
                ...
                double d12 = this.mc.vrPlayer.vrdata_room_pre.getHeadPivot().y;
                double d1 = this.mc.vrPlayer.vrdata_room_pre.getController(this.latchStartController).getPosition().y;
                ...
                {
                    Vec3 vec32 = this.mc.vrPlayer.vrdata_world_pre.hmd.getDirection().scale((double)0.1F);
            ...
            else
            {
                ...
                Vec3 vec39 = this.mc.vr.controllerHistory[this.latchStartController].netMovement(0.3D);
                double d5 = this.mc.vr.controllerHistory[this.latchStartController].averageSpeed((double)0.3F);
                ...
                vec39 = vec39.yRot(this.mc.vrPlayer.vrdata_world_pre.rotation_radians);
                ...
                this.mc.vrPlayer.snapRoomOriginToPlayerEntity(player, false, false);
