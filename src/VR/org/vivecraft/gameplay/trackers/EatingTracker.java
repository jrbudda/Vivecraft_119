...
public class EatingTracker extends Tracker
{
	...
	public void doProcess(LocalPlayer player)	{
		...
		for (int c = 0; c < 2; ++c)	{
			...
			Vec3 controllerPos = this.mc.vr.controllerHistory[c].averagePosition(0.333D).add(vrplayer.vrdata_room_pre.getController(c).getCustomVector(new Vec3(0.0D, 0.0D, -0.1D)));
			controllerPos = controllerPos.add(this.mc.vrPlayer.vrdata_room_pre.getController(c).getDirection().scale(0.1D));
			...
			if (mouthPos.distanceTo(controllerPos) < (double)this.threshold)	{
				...
				if (this.eating[c])
				{
					...
					{
						this.mc.vr.triggerHapticPulse(c, 700);
					}
				}
