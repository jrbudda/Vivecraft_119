...
public class CameraTracker extends Tracker
{
    ...
    public boolean isActive(LocalPlayer player)
    {
        ...
        else if (this.mc.vrSettings.seated)
        {
            return false;
        }
        else
    ...
    public void doProcess(LocalPlayer player)
    {
        ...
        {
            VRData.VRDevicePose vrdata$vrdevicepose = this.mc.vrPlayer.vrdata_world_render.getController(this.startController);
        ...
        if (this.quickMode && !this.isMoving() && !this.mc.grabScreenShot)
        {
            this.visible = false;
        }

        if (this.mc.vrPlayer.vrdata_world_render.getEye(RenderPass.CENTER).getPosition().distanceTo(this.position) > (double)(this.mc.options.renderDistance().get() * 12))
        {
            this.visible = false;
        }
    }
    ...
    public void startMoving(int controller, boolean quickMode)
    {
        ...
        this.startControllerPose = this.mc.vrPlayer.vrdata_world_pre.getController(controller);
